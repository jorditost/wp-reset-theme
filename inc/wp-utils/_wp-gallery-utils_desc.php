...
function the_post_gallery( $size = 'large', $show_videos = false, $tag = 'li', $exclude_thumb = true, $more_attr = '', $max_images = -1 ) {

	echo get_post_gallery( $size, $show_videos, $tag, $exclude_thumb, $more_attr, $max_images );
}

function get_post_gallery( $size = 'large', $show_videos = false, $tag = 'li', $exclude_thumb = true, $more_attr = '', $max_images = -1 ) {
...
function get_post_gallery_by_ID($post_id, $size = 'large', $show_videos = false, $tag = 'li', $exclude_thumb = true, $more_attr = '', $max_images = -1 ) {
	...
	// Get Images
	$images =& get_children( 'post_type=attachment&post_mime_type=image&post_parent='.$post_id.'&exclude='.$thumb.'&numberposts='.$max_images.'&order=ASC&orderby=menu_order');
	...
	if (is_array( $arrKeys )) {
		...
		foreach ( (array) $arrKeys as $attachment_id ) {			
			...
			$result .= ( !empty($tag) ) ? '<'.$tag.' class="image"'.$more_attr.'> <span class="infoPosition"><span class="infoArea"><h3>'.$img_title.'</h3><span class="infotext">'.$img_description.'</span></span></span>' . $img_element . '</'.$tag.'>' : $img_element;
		}
		unset($attachment_id);
	}
...
function get_post_img_gallery_src_array( $post_id, $exclude_thumb = true ) {
	...
	if (is_array( $arrKeys )) {
		$src_array = array();
		foreach ( (array) $arrKeys as $attachment_id ) {
			$thumb_url = wp_get_attachment_image_src($attachment_id, 'large'); // Full size
